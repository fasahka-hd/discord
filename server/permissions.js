...
export function member(guildId,userId){return get('SELECT * FROM guild_members WHERE guild_id=? AND user_id=?',[guildId,userId])}
export function permissionsFor(guildId,userId){
  const m=member(guildId,userId);if(!m)return new Set()
  if(m.role==='owner'||m.role==='admin')return new Set(['ADMINISTRATOR',...PERMISSIONS])
  ...
  const out=new Set();for(const row of roles)for(const p of String(row.permissions||'').split(','))if(PERMISSIONS.has(p))out.add(p);return out
}
export function hasPermission(guildId,userId,permission){const p=permissionsFor(guildId,userId);return p.has('ADMINISTRATOR')||p.has(permission)}
export function canUseChannel(guildId,userId,channelId,permission='VIEW_CHANNEL'){
  const m=member(guildId,userId);if(!m)return false
  if(m.role==='owner'||m.role==='admin')return true
  let allowed=hasPermission(guildId,userId,permission)
  ...
  for(const row of rows){if(String(row.deny_permissions||'').split(',').includes(permission))allowed=false;if(String(row.allow_permissions||'').split(',').includes(permission))allowed=true}
  return allowed
}
