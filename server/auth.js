...
export function authMiddleware(req,res,next) {
  ...
  if(restriction && !(req.method==='POST'&&req.path==='/logout'))return res.status(403).json({error:restriction.action==='banned'?'Учётная запись заблокирована':'Учётная запись приостановлена',code:restriction.action,until:restriction.until||null})
  req.user=user;next()
}
export function publicUser(u){
  if(!u)return null
  let badges=[];try{badges=u.badges?JSON.parse(u.badges):[]}catch{}
...
export function register(username,password){
  ...
  const id=uid();run('INSERT INTO users (id,username,discriminator,password_hash,bio,status,created_at) VALUES (?,?,?,?,?,?,?)',[id,username,'0',bcrypt.hashSync(password,12),'','online',now()]);return get('SELECT * FROM users WHERE id=?',[id])
}
export function login(username,password){
  ...
  const r=restrictionOf(u);if(r?.action==='banned')throw httpError(403,'Учётная запись заблокирована администратором.');if(r?.action==='suspended')throw httpError(403,`Учётная запись приостановлена до ${new Date(r.until).toLocaleString('ru-RU')}.`);return u
}
